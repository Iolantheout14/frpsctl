...
| 服务 | **服务视图**（v0.3.4）：frps / Web 管理台 / 服务端插件的托管状态；插件 `start|stop|restart` 可在页面操作（Web 自身只读，见边界） |
| 版本 | **版本管理**（v0.3.4）：运行中/磁盘版本与一致性；表单提交安装任务（后台执行 + 进度轮询）；"重启使新版本生效"入口 |
| 交互 | **命令面板 `Ctrl+K`**（视图/动作/主题/复制）、快捷键 `g d/c/a/s/v`、`r` 刷新、`/` 聚焦过滤、表格排序与过滤、**客户端/代理详情抽屉**（v0.3.4） |
...
```bash
uv venv && uv pip install -e ".[dev]"
.venv/bin/pytest                       # 全部 922 条（契约层缺二进制时自动 skip）
.venv/bin/pytest -m "not contract"     # 快速回归（892 条）
...
| **Web 不做 DOM 级前端测试框架** | 引入 jsdom/截图框架会破坏"零外部域"这个安全资产；语法与静态纪律（含 **import 图**：路径存在/符号有导出/无孤儿模块）由 CI 守卫，**纯逻辑模块由 `node --test` 直接 import 断言**（v0.3.4 起 24 条），DOM 分支仍靠人工点验（残余风险已记账） |
