...
| 项 | 状态 |
|----|------|
| 代码 | `src/frpsctl/` 共 **4910 行**（14 个模块 + CLI） |
| 测试 | **89 个用例**，1912 行；单元 / 集成 / CLI / 契约四层 |
| 真机验证 | frps **0.71.0** 全链路冒烟通过：`init → verify → start → status → config set（自动重启）→ doctor → config rollback → stop` |
